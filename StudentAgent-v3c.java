public class StudentAgent {
  ...
  public static TreasureHunt.TrialResult run(TreasureHunt game) {
    ...
    if(!item.isHint()) return game.submit();
  ...
  public static int[] lookForHint(TreasureHunt game) {
    ...
    for(int checkDist = 30; checkDist >= 0; checkDist--) {
      for(int x = 0; x < 20; x++) {
        for(int y = 0; y < 20; y++) {
          for(int z = 0; z < 20; z++) {
            int manDist = Math.abs(9 - x) + Math.abs(9 - y) + Math.abs(9 - z);
            if(manDist == checkDist) {
              if(jump(x, y, z, game)) return hint;
              goalCandidate[x][y][z] = false;
              hintCandidate[x][y][z] = false;
              if(game.search() != null) { 
                hint[0] = x;
  ...
  public static void updateGoalRegion(TreasureHunt.Collectible hint, int[] hintCoord) {
    ...
    //Check if each space within the cube is in the goalCandidate region given by the hint
    //Do this by checking furthest points from the center and circling in (based on manhattan distance)
    for(int x = 0; x < 20; x++) {
      for(int y = 0; y < 20; y++) {
        for(int z = 0; z < 20; z++) {
